import { ttCommentsData } from './state';
import { showError } from './ui';

let _wcDebounce: ReturnType<typeof setTimeout> | null = null;

export function ttGetWordCount(): number {
  const el = document.getElementById('tt-wc-words') as HTMLInputElement;
...
export function ttGetFontScale(): number {
  const el = document.getElementById('tt-wc-font') as HTMLInputElement;
...
export function ttRegenerateWordCloud(): void {
...
function ttBuildWordFreq(limit: number): { text: string; value: number }[] {
  const bigram  = (document.getElementById('tt-wc-bigram')  as HTMLInputElement)?.checked ?? false;
  const trigram = (document.getElementById('tt-wc-trigram') as HTMLInputElement)?.checked ?? false;
...
export function ttGenerateWordCloud(force = false): void {
  if (!ttCommentsData.length) { showError('Fetch comments first.'); return; }

  const wrap = document.getElementById('tt-wordcloud-wrap')!;
  const canvas = document.getElementById('tt-wordcloud-canvas') as HTMLCanvasElement;
...
export function ttDownloadWordCloud(): void {
  const canvas = document.getElementById('tt-wordcloud-canvas') as HTMLCanvasElement;
...
export function ttCopyWordCloud(): void {
  const canvas = document.getElementById('tt-wordcloud-canvas') as HTMLCanvasElement;
  canvas.toBlob(blob => { if (!blob) return; navigator.clipboard.write([new ClipboardItem({ 'image/png': blob })]); });
  showError('Copied!');
}

export function _wcUpdateSliderFill(el: HTMLInputElement | null): void {
  if (!el) return;
...
export function _wcLiveUpdate(): void {
  if (!ttCommentsData.length) return;
  ttGetWordCount(); ttGetFontScale();
  _wcUpdateSliderFill(document.getElementById('tt-wc-words') as HTMLInputElement);
  _wcUpdateSliderFill(document.getElementById('tt-wc-font') as HTMLInputElement);
