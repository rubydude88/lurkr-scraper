import { igCommentsData } from './state';
import { _wcUpdateSliderFill } from './wordcloud';
import { showError } from './ui';

let _igWcDebounce: ReturnType<typeof setTimeout> | null = null;

export function igGetWordCount(): number {
  const el = document.getElementById('ig-wc-words') as HTMLInputElement;
...
export function igGetFontScale(): number {
  const el = document.getElementById('ig-wc-font') as HTMLInputElement;
...
export function igRegenerateWordCloud(): void {
...
function igBuildWordFreq(limit: number): { text: string; value: number }[] {
  const bigram  = (document.getElementById('ig-wc-bigram')  as HTMLInputElement)?.checked ?? false;
  const trigram = (document.getElementById('ig-wc-trigram') as HTMLInputElement)?.checked ?? false;
...
export function igGenerateWordCloud(force = false): void {
  if (!igCommentsData.length) { showError('Fetch Instagram comments first.'); return; }

  const wrap = document.getElementById('ig-wordcloud-wrap')!;
  const canvas = document.getElementById('ig-wordcloud-canvas') as HTMLCanvasElement;
...
export function igDownloadWordCloud(): void {
  const canvas = document.getElementById('ig-wordcloud-canvas') as HTMLCanvasElement;
...
export function igCopyWordCloud(): void {
  const canvas = document.getElementById('ig-wordcloud-canvas') as HTMLCanvasElement;
...
export function _igWcLiveUpdate(): void {
  if (!igCommentsData.length) return;
  igGetWordCount(); igGetFontScale();
  _wcUpdateSliderFill(document.getElementById('ig-wc-words') as HTMLInputElement);
  _wcUpdateSliderFill(document.getElementById('ig-wc-font') as HTMLInputElement);
